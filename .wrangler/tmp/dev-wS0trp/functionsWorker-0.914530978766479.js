...
var onRequestPost2 = /* @__PURE__ */ __name2(async (context) => {
  ...
  if (!auth.ok) {
    return new Response(auth.message, { status: auth.status, headers: cors2(origin) });
  }
  try {
    const db = getDb2(env);
    const body = await request.json().catch(() => ({}));
...
var onRequestGet2 = /* @__PURE__ */ __name2(async (context) => {
  ...
  const auth = await getAuthContext(request, env);
  if (!auth.ok) {
    return new Response(auth.message, { status: auth.status, headers: cors3(origin) });
  }
  try {
    const db = getDb2(env);
    ...
    return Response.json({ ok: true, items }, { headers: cors3(origin) });
  } catch (e) {
    return Response.json(
      { ok: false, error: e?.message || String(e), stack: e?.stack || null },
      { status: 500, headers: cors3(origin) }
    );
  }
}, "onRequestGet");
...
// .wrangler/tmp/bundle-pVp9nJ/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-pVp9nJ/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.914530978766479.js.map
