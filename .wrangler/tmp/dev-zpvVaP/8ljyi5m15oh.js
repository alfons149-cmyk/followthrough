...
// .wrangler/tmp/pages-p5D7hs/functionsWorker-0.6658554450379071.mjs
...
async function getAuthContext(env, request) {
  ...
  const db = getDb(env);
  const rows = await db.select().from(apiKeys).where(and(eq(apiKeys.keyHash, keyHash), isNull(apiKeys.revokedAt))).limit(1);
  const row = rows[0];
...
var cors3 = /* @__PURE__ */ __name2((origin) => ({
  "Access-Control-Allow-Origin": origin || "*",
...
function daysOverdue(dueAt) {
  ...
  const dueDate = /* @__PURE__ */ new Date(due + "T00:00:00Z");
...
var onRequestGet2 = /* @__PURE__ */ __name2(async ({ env, request }) => {
  ...
  const where = status && status !== "all" ? and(baseWhere, eq(followups.status, status)) : baseWhere;
...
// .wrangler/tmp/pages-p5D7hs/8ljyi5m15oh.js
...
// .wrangler/tmp/bundle-Be5Wdf/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Be5Wdf/middleware-loader.entry.ts
...
//# sourceMappingURL=8ljyi5m15oh.js.map
